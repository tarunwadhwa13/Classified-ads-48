...
router.post('/id/:id/contact',
    ...
    async function(req, res, next) {
      logger_.log({level: 'info', message: `listing id:: ${req.params.id}`});
      const hex = /[0-9A-Fa-f]{6}/g;
      const elem = (hex.test(req.params.id))?
      await mongoQueries.getDocumentById(global.mongodb, req.params.id, false) :
        undefined;
      ...
      if (!valid) {
        ...
        logger_.log({level: 'info', message: `reciever:: ${elem.usr}`});
        logger_.log({level: 'info', message: `SENDER:: ${req.usr}`});
...
router.get('/user',
    ...
    async function(req, res, next) {
      const listings = await mongoQueries.getDocumentsByUser(
          global.mongodb, req.oidc.user.email);
      res.render(
          'listings',
          {
            title: 'Your listings',
