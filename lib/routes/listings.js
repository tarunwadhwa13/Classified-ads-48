...
router.get('/id/:id/', makeRenderer, async function (req, res, next) {
...
router.post('/gwoogl', makeRenderer, async (req, res, next) => {
  const { body } = req
...
router.post('/geolocation', makeRenderer, async (req, res) => {
  const { body } = req
  const querySchema = Joi.object().keys({
...
router.post(/^\/(donations|skills|blogs)/,
  ops.multer, secure, makeRenderer,
  async (req, res, next) => {
    const { body } = req
    const section = req.params[0]
...
router.get('/autocomplete/:keyword', makeRenderer, async function (req, res, next) {
