...
router.get('/admin', async function (req, res, next) {
  ...
  // logger.log({ level: 'info', message: `Raw data ${JSON.stringify(realtimeJSON.slice(0,10))}`  })
...
router.delete('/admin/:id', function(request, response) {
  ...
  const match = getMatch(request);
  if (match !== undefined) realtimeJSON.splice(match, 1);
  response.send(realtimeJSON);
});

function getMatch(req) {
  return realtimeJSON.reduce((acc, curr, index) =>
      curr._id.toString() === req.params.id.toString() );
}
