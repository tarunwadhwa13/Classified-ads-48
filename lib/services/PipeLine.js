...
PipeLine.prototype = {
    ...
    joiValidate: function (schema, op) {
        const predicate = (schema) => {
            ...
            let res = schema.def.validate(this.data)
...
function validationPipeLine(req) {
    ...
    const undrawValid = !illustrations ? true : new PipeLine(body)
        .undrawSplit().joiValidate(singletonSchema).postValidate()
...
// Middlewares
function makeRenderer (req, res, next) {
...
module.exports = { validationPipeLine, stringTransformer, makeRenderer, secure }