 ...
 */
 queries.getMessages = async function (peer1, peer2, thread) {
  ...
  return await db.collection('user').find(query).sort({sent:-1}).toArray()
}
 ...
 * @return {Promise}
 */
 queries.getDocumentsForApproval = async function () {
  db = await getDB()
  const collection = db.collection('listing')
...
queries.gwoogl = async function (phrase, exact, division, section) {
  ...
  return await collection.find(query, { score: { $meta: 'textScore' } })
      .project(baseProjection)
...
queries.topTags = async function() {
  ...
  topTags.sort(function (x, y) {
    return ('' + x._id.section).localeCompare(y._id.section)
  })
  return topTags
}
...
module.exports.mongoQueries = traceMethodCalls(queries);
