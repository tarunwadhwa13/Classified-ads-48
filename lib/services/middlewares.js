...
PipeLine.prototype = {
    ...
    joiValidate: function (schema, op) {
        const predicate = (schema) => {
            return schema.validate(this.data.body)
        }
 ...
 * @param {*} body 
 */
function getMiddlewares(method, section, body) {
    const {
        secured,
        upload,
        geolocation,
    ...
    const undrawValid = !illustrations ? true : new PipeLine({
        body: body
    }).undrawSplit().joiValidate(schema).undrawJoin().evaluate()
    const bodyValid = [geoValid, undrawValid].every(Boolean)
    return { middlewares: [uploading, securing], bodyValid: bodyValid}
}