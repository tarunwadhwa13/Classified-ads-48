 ...
 * @return {Promise}
 */
queries.insertDocument = async function(loggerelem) {
 ...
 * @return {Promise}
 */
queries.insertMessage = async function(loggerelem) {
  console.log('insertDocument');
 ...
 * @return {Promise}
 */
queries.getDocumentById = async function(loggerid, isAdmin) {
  const collection = db.collection('listing');
 ...
 * @return {Promise}
 */
queries.getDocumentsSince = function(loggerdays, section, pagination) {
  const collection = db.collection('listing');
 ...
 * @return {Promise}
 */
queries.getDocumentsByUser = function(loggeruser) {
  const collection = db.collection('listing');
  const query = {};
  query['usr'] = user;
  return new Promise(function(resolve, reject) {
 ...
 * @return {Promise}
 */
queries.gwoogl = function(loggerphrase, exact, division, section) {
  const collection = db.collection('listing');
 ...
 * @return {Promise}
 */
queries.getDocumentsByTag = function(loggertag, pagination) {
  const collection = db.collection('listing');
  const objectId = getObjectId(100);
  const query = JSON.parse(JSON.stringify(baseQuery));
  query['_id'] = {$gt: objectId};
 ...
 * @return {Promise}
 */
queries.getDocumentsByDivision = function(loggerdivision, pagination) {
  const collection = db.collection('listing');
  const objectId = getObjectId(100);
  const query = JSON.parse(JSON.stringify(baseQuery));
  query['_id'] = {$gt: objectId};
 ...
 * @return {Promise}
 */
queries.getDocumentsByGeolocation = function(loggerlatitude, longitude, section) {
  const collection = db.collection('listing');
  const objectId = getObjectId(100);
  const query = JSON.parse(JSON.stringify(baseQuery));
  query['_id'] = {$gt: objectId};
...
queries.deactivateDocument = function(loggerpassword) {
  const collection = db.collection('listing');
  const query = JSON.parse(JSON.stringify(baseQuery));
  query['pass'] = password;
...
queries.approveDocument = function(loggerid) {
  const collection = db.collection('listing');
  const query = JSON.parse(JSON.stringify(baseQuery));
  query['_id'] = new ObjectID(id);
  query['a'] = false;
...
// queries.getDocumentId = function(loggerquery, cb) {
//   const collection = db.collection('listing');
...
queries.reactivateDocument = function(loggerpassword) {
  const collection = db.collection('listing');
  const query = JSON.parse(JSON.stringify(baseQuery));
  query['pass'] = password;
  query['d'] = true;
...
queries.autocomplete = function(loggerkeyword) {
...
queries.getDocumentsByKeyword = function(loggerkeyword, pagination) {
  const collection = db.collection('words');
  return new Promise(function(resolve, reject) {
